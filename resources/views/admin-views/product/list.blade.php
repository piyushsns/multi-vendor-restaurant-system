        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-filter-list"></i> {{ __('messages.product') }} {{ __('messages.list') }}<span
                            class="badge badge-soft-dark ml-2" id="productCount"></span></h1>
                </div>
                    ...
                    <div class="card-header p-1">
                        <div class="row justify-content-between align-items-center flex-grow-1">
                            <div class="col-md-4 mb-3 mb-md-0">
                                <h5 style="margin-left:12px;">{{__('messages.product')}} {{__('messages.list')}}<span class="badge badge-soft-dark ml-2" id="itemCount">{{$products->total()}}</span></h5>
                            </div>
                            @csrf
                            <div class="col-auto">
                                ...
                                <tbody id="set-rows">
                                    @foreach ($products as $key => $product)
                                        <tr>
                                            ...
                                            <td>
                                                <a class="media align-items-center" href="{{ route('admin.product.view', [$product['id']]) }}"><h5 class="text-hover-primary mb-0">{{ Str::limit($product['name'], 35, '...') }}</h5></a>
                                            </td>
                                            <td>
                                                {{ Str::limit($product->category, 20, '...') }}
                                            </td>
                                            <td>
                                                {{ Str::limit($product->restaurant ? $product->restaurant->name : __('messages.Restaurant deleted!'), 20, '...') }}
                                            </td>
                                            <td>
                                                {{ \App\CentralLogics\Helpers::format_currency($product['price']) }}
                                            </td>
                                            <td>
                                                <form action="{{route('admin.product.change-priority',$product->id)}}">
                                                    <select name="priority" id="priority" onchange="this.form.submit()"> 
                                                        <option value="0" {{$product->priority == 0?'selected':''}}>Normal</option>
                                                        <option value="1" {{$product->priority == 1?'selected':''}}>Priority</option>
                                                    </select>
                                                </form>
                                            </td>
                                            <td>
                                                <div class="row text-center">
                                                    <label class="toggle-switch toggle-switch-sm col-sm-auto" for="stocksCheckbox{{$product['id']}}f_featured">
                                                        <input type="checkbox"
                                                            onclick="feature_change_alert('{{route('admin.product.featured',[$product,$product['f_featured']==0?1:0])}}')" 
                                                            class="toggle-switch-input" id="stocksCheckbox{{$product['id']}}f_featured" {{$product['f_featured'] == 1?'checked':''}}>
                                                            {{__('messages.feature')}}&nbsp;&nbsp;&nbsp;
                                                            <span class="toggle-switch-label">
                                                            <span class="toggle-switch-indicator"></span>
                                                        </span>
                                                    </label>
                                                    <label class="toggle-switch toggle-switch-sm col-sm-auto" for="stocksCheckbox{{$product['id']}}f_trending" >
                                                        <input type="checkbox"
                                                            onclick="feature_change_alert('{{route('admin.product.trending',[$product['id'],$product['f_trending']==0?1:0])}}')" 
                                                            class="toggle-switch-input trending-toggle" id="stocksCheckbox{{$product['id']}}f_trending" {{$product['f_trending'] == 1 ? 'checked':''}}>
                                            ...
                                            <td>
                                                <a class="btn btn-sm btn-white"
                                                    ...
                                                    title="{{ __('messages.edit') }} {{ __('messages.product') }}"><i
                                                        class="tio-edit"></i>
                                                </a>
                                                <a class="btn btn-sm btn-danger" href="javascript:"
                                                    onclick="form_alert('product-{{ $product['id'] }}','{{ __('messages.Want_to_delete_this_item') }}')"
                                                    title="{{ __('messages.delete') }} {{ __('messages.product') }}"><i
